...
/**
 * Which sections each team sees, in the order its workflow runs. Everyone gets
 * the status header + next-step panel first; then only the sections relevant to
 * their job, so the drawer reads as that team's checklist rather than a dump of
 * every control. Sections that don't apply (e.g. an offer letter for a non-
 * English track) resolve to null and drop out.
 */
const SECTION_ORDER: Record<string, string[]> = {
  admin: ["application", "documents", "studyPlan", "offerLetter", "fees", "visa", "requests", "message", "workLog", "activity"],
  admissions: ["application", "documents", "studyPlan", "offerLetter", "fees", "visa", "requests", "message", "workLog", "activity"],
  visa: ["visa", "application", "documents", "workLog", "requests", "activity"],
  academic: ["studyPlan", "application", "documents", "fees", "workLog", "requests", "activity"],
  counsellor: ["application", "documents", "message", "requests", "workLog", "activity"],
  staff: ["application", "documents", "activity"],
};
...
export function ApplicationDrawer({
  ...
  const canEditDocs = ["admin", "admissions", "visa"].includes(role);
  const canGenerateOffer = app.track === "english" && ["admin", "admissions"].includes(role);
  const showFees = fees.length > 0 || (billables.length > 0 && ["admin", "finance"].includes(role));
  const showVisa = Boolean(visa) || (app.is_international && ["admin", "visa"].includes(role));
  ...
  const sections: Record<string, React.ReactNode> = {
    ...
    documents: (
      <div>
        ...
        />
        {canEditDocs && <DocRequestControl applicationId={app.id} requests={docRequests} />}
      </div>
    ),
    ...
    studyPlan:
      app.track !== "corporate" ? (
        <div>
          ...
          <PlanEditor applicationId={app.id} studentName={app.student_name} plan={app.plan} role={role} />
    ...
    fees: showFees ? (
      <div>
          ...
          {fees.map((f) => (
            <div key={f.id} className="flex items-center justify-between text-sm">
              ...
              <span className="flex items-center gap-2">
                <span className="font-mono text-xs text-ink tabular">{formatMoney(f.amount, f.currency)}</span>
                <span
        ...
        {["admin", "finance"].includes(role) && <AddFeeControl applicationId={app.id} items={billables} />}
      </div>
    ) : null,
    ...
    visa: showVisa ? (
      visa ? (
        ...
        <div>
          <SectionLabel>Visa / EMGS</SectionLabel>
          <p className="mb-2 text-sm text-ink-muted">No visa case yet. Start one to track EMGS / student pass:</p>
    ...
    requests: (
      <div>
              ...
              .map((r) => (
                <div
                  ...
                  className={`rounded-md border px-3 py-2 ${
                    r.type === "blocker" ? "border-brand-red/50 bg-brand-red-bg/40" : "border-border-warm bg-paper"
                  }`}
                >
        ...
        <RaiseRequest applicationId={app.id} subject={app.student_name} fromRole={role} />
      </div>
    ),
    ...
    message: (
      <div>
        ...
        <p className="mb-3 text-xs text-ink-muted">Pick a template, edit if needed, then send via WhatsApp or email.</p>
        <MessageComposer
          recipient={{ name: app.student_name, email: contact?.email ?? app.student_email, phone: contact?.whatsapp ?? contact?.phone }}
          vars={messageVars}
          context="application"
          team={role}
          onSent={(channel, label) => start(async () => { await logApplicationMessage(app.id, channel, label); router.refresh(); })}
        />
      </div>
    ),
      ...
      <div>
        ...
        <div className="flex flex-col gap-2">
          ...
          <button
            ...
            onClick={() => start(async () => { await addApplicationNote(app.id, note); setNote(""); router.refresh(); })}
    ...
    <div className="fixed inset-0 z-40 flex justify-end">
      ...
      <aside className="relative z-10 flex h-full w-full max-w-md flex-col overflow-y-auto bg-cream shadow-lg">
        <div className="flex items-start justify-between border-b border-border-warm px-6 py-4">
          <div>
            <h2 className="font-serif text-2xl font-medium text-ink">{app.student_name}</h2>
        ...
        <div className="flex flex-col gap-6 px-6 py-5">
          {/* Status + guided next step — shown to everyone, first. */}
          <div className="flex items-center gap-2">
            <span
              className={`inline-flex rounded-md px-2.5 py-1 text-xs font-medium ${
                app.is_international ? "bg-brand-gold/20 text-brand-gold" : "bg-cream-50 text-ink-muted"
              }`}
            >
