 ...
 */
export function AgentRegisterModal({ agentCode, lang = "en" }: { agentCode: string; lang?: ConsoleLang }) {
  ...
  return (
    <>
      ...
      {open && (
        <div dir={lang === "ar" ? "rtl" : "ltr"} className="fixed inset-0 z-50 flex items-start justify-center overflow-hidden bg-inkbtn/40 p-3 sm:p-6">
          ...
          <div className="relative z-10 flex h-full max-h-[92vh] w-full max-w-2xl flex-col overflow-hidden rounded-card border border-border-warm bg-cream shadow-xl">
            <div className="flex items-center justify-between border-b border-border-warm bg-paper px-4 py-2.5">
              <p className="text-sm font-medium text-ink">{L.ag_refer_student}</p>
              <button onClick={close} aria-label={L.close} className="rounded-md p-1 text-ink-muted hover:bg-cream-50 hover:text-ink">
                <X className="h-5 w-5" aria-hidden />
              </button>
            </div>
            <iframe
              src={`/register?agent=${encodeURIComponent(agentCode)}&embedded=1`}
              title={L.ag_refer_student}
