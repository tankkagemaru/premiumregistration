...
export function ClaimInvoiceUpload({
  ...
  if (submitted) {
    return (
      <span className="inline-flex items-center gap-1 text-[11px] font-medium text-status-present">
        <Check className="h-3 w-3" aria-hidden /> {L.ag_claim_submitted}
      </span>
    );
  }
  if (!claimReady) {
    return <span className="text-[11px] text-ink-muted">{L.ag_claim_opens}</span>;
  }
  ...
  function upload(file: File) {
    ...
    start(async () => {
      ...
      if ("error" in res) { setError(L.ag_upload_unavailable); return; }
      const { createClient } = await import("@/lib/supabase/client");
      const supabase = createClient();
      const { error: upErr } = await supabase.storage
        .from(BUCKET)
        .uploadToSignedUrl(res.path, res.token, file);
      if (upErr) { setError(L.ag_upload_failed); return; }
