...
export async function createApplicationFromLead(leadId: string) {
  ...
  for (const track of tracks) {
    ...
    if (createdApp && !firstAppId) firstAppId = createdApp.id as string;
  }

  // Scaffold the fees marketing quoted (what we're selling) onto the first
  // application. Fees are finance-write under RLS, so use the service role.
  const quote = (reg.details?.quote ?? []) as {
    name: string; fee_type: string; amount: number; currency: string;
  }[];
  if (firstAppId && quote.length) {
